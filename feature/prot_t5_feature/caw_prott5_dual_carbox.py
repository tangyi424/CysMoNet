...
input_pos = "/home/lichangyong/documents/tangyi/SulMoNet/data_2/all_pos_2.txt"
input_neg = "/home/lichangyong/documents/tangyi/SulMoNet/data_2/all_neg_2.txt"
out_dir = "/home/lichangyong/documents/tangyi/SulMoNet/data_2/"
os.makedirs(out_dir, exist_ok=True)

batch_size = 4
window = 41                # ✅ 改为 41
center_idx = window // 2   # ✅ 中心索引变为 20
...
def extract_cys_window(seq, window=41):
    """提取以第一个 Cys (S-Carboxyethylation 位点) 为中心的窗口"""
...
def compute_caw_from_attn(attn_all, center_idx=20):
    """计算对称注意力矩阵 + 中心导向注意力曲线"""
    A = attn_all.mean(dim=(0, 1))  # 平均所有层与head
    A_sym = (A + A.T) / 2
...
def compute_mean_attention(seqs, label, save_prefix):
    """计算平均注意力矩阵与CAW曲线"""
    A_list, w_list = [], []
    for i in tqdm(range(0, len(seqs), batch_size), desc=f"[{label}] batches"):
        batch = seqs[i:i + batch_size]
        batch_spaced = [" ".join(list(s)) for s in batch]
        tokens = tokenizer(batch_spaced, return_tensors="pt", padding=True, truncation=True, max_length=window).to(device)

        with torch.no_grad():
            outputs = model(**tokens, output_attentions=True)
            attns = torch.stack(outputs.attentions)  # [num_layers, B, num_heads, L, L]
...
def plot_dual_results(A_pos, w_pos, A_neg, w_neg, save_prefix):
    ...
    ax1.set_title("ProtT5 Attention Analysis for S-Carboxyethylation Prediction",
    ...
    # 添加模型信息标注
    # ax2.text(0.98, 0.02, "Model: ProtT5 | Task: S-sulfhydration Prediction",
    ...
    ax2.text(
        0.02, 0.5,  # 👈 x=0.02靠近左边，y=0.5垂直居中
        "Model: ProtT5 | Task: S-Carboxyethylation Prediction",
        transform=ax2.transAxes,
        fontsize=9, fontstyle='italic',
        horizontalalignment='left',  # 👈 改为左对齐（靠左框线）
        verticalalignment='center',  # 👈 垂直居中
        bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.5)
    )

    plt.tight_layout()
    plt.savefig(f"{save_prefix}_prot_t5_S-Carboxyethylation_analysis.png",
                dpi=600, bbox_inches='tight', facecolor='white')
    plt.close()

    print(f"✅ Saved figure: {save_prefix}_prot_t5_S-Carboxyethylation_analysis.png")
...
def main():
    ...
    save_prefix = os.path.join(out_dir, "CAW_Scarboxy")

    A_pos, w_pos = compute_mean_attention(seqs_pos, "pos", save_prefix)
    A_neg, w_neg = compute_mean_attention(seqs_neg, "neg", save_prefix)

    plot_dual_results(A_pos, w_pos, A_neg, w_neg, save_prefix)

    print("✅ All done for S-Carboxyethylation analysis.")
